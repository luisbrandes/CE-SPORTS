...
import { Pencil, Trash2, CalendarDays, MapPin, Users, Search, Filter, X } from "lucide-react";
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
          {projetosFiltrados.length > 0 ? (
            projetosFiltrados.map((proj) => {
              const progress =
                proj.vagasTotais > 0
                  ? (proj.vagasPreenchidas / proj.vagasTotais) * 100
                  : 0;
              ...
              return (
                ...
                >
                  <div className="p-4 sm:p-6 space-y-4">
                    ...
                    <div className="space-y-2 pt-1">
                      ...
                      <div className="flex items-center justify-between text-[11px] text-[#A0A7B8]">
                        <span>
                          {proj.vagasPreenchidas}/{proj.vagasTotais} vagas
                        </span>
                  ...
                  {/* Botões: sempre alinhados e responsivos */}
                  <div className="border-t border-[#F0F1F6] px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      className="flex-1 h-9 sm:h-10 border-[#D2D7E5] text-[#14539A] hover:bg-[#F2F4FB] text-xs sm:text-sm"
                      onClick={() =>
                        router.push(`/admin/projetos/editar/${proj.id}`)
                      }
                    >
                    ...
                    <Button
                      ...
                      className="flex-1 h-9 sm:h-10 border-[#D2D7E5] text-[#14539A] hover:bg-[#F2F4FB] text-xs sm:text-sm"
                      onClick={() =>
                        router.push(`/admin/projetos/${proj.id}/inscritos`)
                      }
                    >
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      className="flex-1 h-9 sm:h-10 text-xs sm:text-sm bg-[#EF4444] hover:bg-[#dc2626]"
